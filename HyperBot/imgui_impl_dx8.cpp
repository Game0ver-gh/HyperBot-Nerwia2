...
//I havent bother to fix frozing client on device reset but you could do so
//by follwing this guide: https://metin2.dev/topic/31240-imgui-library-support/
...
static bool ImGui_ImplD3D8_CreateDepthStencilBuffer() {
    ...
    if (g_DepthBuffer == nullptr) {
        ...
        g_pd3dDevice->GetDepthStencilSurface(&realDepth); //nullptr
        if (!realDepth || realDepth->GetDesc(&sfcDesc) != D3D_OK)
        {
            D3DVIEWPORT8 vp;
            if (g_pd3dDevice->GetViewport(&vp) != D3D_OK)
                return false;

            if (g_pd3dDevice->CreateDepthStencilSurface(vp.Width, vp.Height, D3DFMT_D24S8, D3DMULTISAMPLE_NONE, &g_DepthBuffer) != D3D_OK)
                return false;
            return true;
        }
        
        if (g_pd3dDevice->CreateDepthStencilSurface(sfcDesc.Width, sfcDesc.Height, D3DFMT_D24S8, D3DMULTISAMPLE_NONE, &g_DepthBuffer) != D3D_OK)
            return false;
