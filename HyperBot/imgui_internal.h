...
{
    ImGuiWindow*        Window;         // CreateDump,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
    ImGuiID             ID;             // CreateDump,Move    // Best candidate item ID
    ImGuiID             FocusScopeId;   // CreateDump,Move    // Best candidate focus scope ID
    ImRect              RectRel;        // CreateDump,Move    // Best candidate bounding box in window relative space
...
struct ImGuiContext
{
    ...
    // Navigation: CreateDump & Move Requests
    bool                    NavAnyRequest;                      // ~~ NavMoveRequest || NavInitRequest this is to perform early out in ItemAdd()
    bool                    NavInitRequest;                     // CreateDump request for appearing window to select first item
    bool                    NavInitRequestFromMove;
    ImGuiID                 NavInitResultId;                    // CreateDump request result (first item of the window, or one for which SetItemDefaultFocus() was called)
    ImRect                  NavInitResultRectRel;               // CreateDump request result rectangle (relative to parent window)
